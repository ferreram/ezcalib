...
public:
  ...
  KB4DistParam(const bool _use_mono_focal)
  ...
  createCeresCostFunction(const double _u, const double _v, const Eigen::Vector3d& _wpt) const override
  {
    if (m_use_mono_focal)
    {
      return  new ceres::AutoDiffCostFunction<
                  AutoDiffKB4CalibMonoFocal_Kernel, 2, 1, 2, 4, 7>(
                      new AutoDiffKB4CalibMonoFocal_Kernel( _u, _v, _wpt));
    }
...
private:
  ...
  struct AutoDiffKB4CalibMonoFocal_Kernel
  {
    AutoDiffKB4CalibMonoFocal_Kernel(const double _u, const double _v,
                                     const Eigen::Vector3d& _wpt)
        : m_px(_u, _v)
        , m_wpt(_wpt)
    {}
