...
public:
  ...
  Rad1DistParam(const bool _use_mono_focal)
    : DistParam(1, _use_mono_focal)
  {}

  Rad1DistParam(const double _k1, const bool _use_mono_focal)
    : DistParam(1, _use_mono_focal)
    , m_k1(_k1)
  {}
  ...
  createCeresCostFunction(const double _u, const double _v, const Eigen::Vector3d& _wpt) const override
  {
    if (m_use_mono_focal)
    {
      return  new ceres::AutoDiffCostFunction<
                  AutoDiffRad1CalibMonoFocal_Kernel, 2, 1, 2, 1, 7>(
                      new AutoDiffRad1CalibMonoFocal_Kernel( _u, _v, _wpt));
    }
...
private:
  ...
  struct AutoDiffRad1CalibMonoFocal_Kernel
  {
    AutoDiffRad1CalibMonoFocal_Kernel(const double _u, const double _v,
                                      const Eigen::Vector3d& _wpt)
        : m_px(_u,_v)
        , m_wpt(_wpt)
    {}
