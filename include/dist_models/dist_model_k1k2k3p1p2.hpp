...
public:
  ...
  RadTan5DistParam(const bool _use_mono_focal)
    : DistParam(5, _use_mono_focal)
  {}
  ...
  createCeresCostFunction(const double _u, const double _v, const Eigen::Vector3d& _wpt) const override
  {
    if (m_use_mono_focal)
    {
      return  new ceres::AutoDiffCostFunction<
                  AutoDiffRadTan5CalibMonoFocal_Kernel, 2, 1, 2, 5, 7>(
                      new AutoDiffRadTan5CalibMonoFocal_Kernel( _u, _v, _wpt));
    }
...
private:
  ...
  struct AutoDiffRadTan5CalibMonoFocal_Kernel
  {
    AutoDiffRadTan5CalibMonoFocal_Kernel(const double _u, const double _v,
                                         const Eigen::Vector3d& _wpt)
        : m_px(_u, _v)
        , m_wpt(_wpt)
    {}
