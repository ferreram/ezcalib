...
public:
  ...
  {
    return {m_k1, m_k2, m_k3, m_p1, m_p2};
  }
...
private:
  ...
  class ReprojectionError : public ceres::SizedCostFunction<2, 2, 2, 5, 7>
  {
  public:
                  ...
                  double **jacobians) const
    {
      ...
      const double k3 = parameters[2][2];
      const double p1 = parameters[2][3];
      const double p2 = parameters[2][4];
      ...
      const double D = 1. + r2*(k1 + k2*r2 + k3*r4);

      const double xd = x*D + p1*xy_2 + p2*(r2 +  2.*x2);
      const double yd = y*D + p2*xy_2 + p1*(r2 +  2.*y2);

      const Eigen::Vector2d pred(fx * xd + cx, 
                                 fy * yd + cy);
      ...
      if (jacobians != nullptr)
      {
        ...
        {
          Eigen::Map<Eigen::Matrix<double, 2, 5, Eigen::RowMajor>> J_dist(jacobians[2]);

          const double fx_x_r2 = fx * x * r2;
          const double fy_y_r2 = fy * y * r2;

          // k1, k2, k3
          J_dist(0,0) = fx_x_r2; J_dist(0,1) = fx_x_r2 * r2; J_dist(0,2) = fx_x_r2 * r4;
          J_dist(1,0) = fy_y_r2; J_dist(1,1) = fy_y_r2 * r2; J_dist(1,2) = fx_x_r2 * r4;

          // p1, p2
          J_dist(0,3) = fx * xy_2;            J_dist(0,4) = fx * (r2 +  2.*x2);
          J_dist(1,3) = fy * (r2 +  2.*y2);   J_dist(1,4) = fy * xy_2;
        }
