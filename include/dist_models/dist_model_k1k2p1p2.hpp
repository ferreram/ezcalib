...
public:
  ...
  RadTan4DistParam(const bool _use_mono_focal)
    : DistParam(4, _use_mono_focal)
  {}
  ...
  void displayParamsWithStd() const override
  {
    ...
    std::cout << "\n Distortion k1 / k2 : " << m_k1 << " / " << m_k2 << " +/- " << m_vdist_params_std[0] << " / " << m_vdist_params_std[1];
    std::cout << "\n Distortion p1 / p2 : " << m_p1 << " / " << m_p2 << " +/- " << m_vdist_params_std[2] << " / " << m_vdist_params_std[3] << "\n\n";
  }
  ...
  createCeresCostFunction(const double _u, const double _v, const Eigen::Vector3d& _wpt) const override
  {
    if (m_use_mono_focal)
    {
      return  new ceres::AutoDiffCostFunction<
                  AutoDiffRadTan4CalibMonoFocal_Kernel, 2, 1, 2, 4, 7>(
                      new AutoDiffRadTan4CalibMonoFocal_Kernel( _u, _v, _wpt));
    }
...
private:
  ...
  struct AutoDiffRadTan4CalibMonoFocal_Kernel
  {
    AutoDiffRadTan4CalibMonoFocal_Kernel(const double _u, const double _v,
                                         const Eigen::Vector3d& _wpt)
        : m_px(_u, _v)
        , m_wpt(_wpt)
    {}
